...
export default function PostButtonWrapper() {
  ...
  const handleDelete = async () => {
    try {
      await $axios.delete(`/api/posts/${objectId}`)
  ...
  return (
    <div className="flex justify-end gap-4 mt-8">
      <button onClick={() => router.push(`/posts/write?id=${objectId}`)} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors shadow-lg hover:shadow-xl">
        수정
      </button>
